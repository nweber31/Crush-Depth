...
# 1 "subSprites.h" 1
# 21 "subSprites.h"
extern const unsigned short subSpritesTiles[8192];


extern const unsigned short subSpritesPal[256];
# 7 "game.c" 2
# 1 "sprites.h" 1
# 10 "sprites.h"
typedef struct {
...
# 8 "game.c" 2
# 1 "reefCollisionMap.h" 1
# 21 "reefCollisionMap.h"
extern const unsigned short reefCollisionMapBitmap[131072];


extern const unsigned short reefCollisionMapPal[256];
# 9 "game.c" 2
# 1 "sound.h" 1
void setupSounds();
...
extern const unsigned int shot_sampleRate;
extern const unsigned int shot_length;
extern const signed char shot_data[];
# 11 "game.c" 2
...
void initGame() {
    ...
    DMANow(3, subSpritesPal, ((u16 *)0x5000200), 256);
    DMANow(3, subSpritesTiles, &((charblock *)0x06000000)[4], 16384 / 2);
    hOff = 50;
    vOff = 50;
    hOff1 = 0;
    vOff1 = 0;
...
void drawJellyfish() {
    for (int i = 0; i < 3; i++) {
        if (jelly[i].y + jelly[i].height > vOff && jelly[i].y < 160 + vOff) shadowOAM[60 + i].attr0 = ((jelly[i].y - vOff) & 0xFF) | (0<<13) | (0<<14);
        shadowOAM[60 + i].attr1 = ((jelly[i].x - hOff) & 0x1FF) | (1<<14);
        shadowOAM[60 + i].attr2 = (((0) & 0xF) << 12) | (((jelly[i].frame * 2) * 32 + (23)) & 0x3FF) | (((0) & 3) << 10);
    }
}
...
void updatePlayer() {
    if ((!(~(oldButtons) & ((1<<9))) && (~(buttons) & ((1<<9)))) && (!(~(oldButtons) & ((1<<8))) && (~(buttons) & ((1<<8)))) && !cheatEnabled) {
        cheatEnabled = 1;
    } else if ((!(~(oldButtons) & ((1<<9))) && (~(buttons) & ((1<<9)))) && (!(~(oldButtons) & ((1<<8))) && (~(buttons) & ((1<<8)))) && cheatEnabled) {
        cheatEnabled = 0;
    }
    int leftX = player.x;
    ...
    if ((~(buttons) & ((1<<7))) && player.y <= 512 - player.height) {
        if (!reverseTimer) {
            player.y += 2;
            vOff1++;
        } else {
    ...
    if ((~(buttons) & ((1<<4))) && player.x <= 512 - player.width) {
        if (!reverseTimer) {
            player.x += 2;
            player.direction = RIGHT;
            hOff1++;
...
void drawPlayer() {
    shadowOAM[0].attr0 = ((player.y - vOff) & 0xFF) | (0<<13) | (1<<14);
    if (player.direction == LEFT) {
        shadowOAM[0].attr1 = ((player.x - hOff) & 0x1FF) | (2<<14) | (1<<12);
    } else {
        shadowOAM[0].attr1 = ((player.x - hOff) & 0x1FF) | (2<<14);
    }
    shadowOAM[0].attr2 = (((0) & 0xF) << 12) | (((0) * 32 + (0)) & 0x3FF) | (((0) & 3) << 10);
}
...
void drawSwordfish() {
    if (swordfishBoss.active) {
        if (swordfishBoss.y + swordfishBoss.height > vOff && swordfishBoss.y < 160 + vOff) {
            shadowOAM[20].attr0 = ((swordfishBoss.y - vOff) & 0xFF) | (0<<13) | (1<<14);
            shadowOAM[21].attr0 = ((sword.y - vOff) & 0xFF) | (0<<13) | (1<<14);
        }
        if (swordfishBoss.direction == RIGHT) {
            shadowOAM[20].attr1 = ((swordfishBoss.x - hOff) & 0x1FF) | (3<<14) | (1<<12);
            shadowOAM[21].attr1 = ((sword.x - hOff) & 0x1FF) | (0<<14) | (1<<12);
        } else {
            shadowOAM[20].attr1 = ((swordfishBoss.x -hOff) & 0x1FF) | (3<<14);
            shadowOAM[21].attr1 = ((sword.x - hOff) & 0x1FF) | (0<<14);
        }
        if (swordfishBoss.hurt) {
            shadowOAM[20].attr2 = (((1) & 0xF) << 12) | (((swordfishBoss.frame * 4) * 32 + (7)) & 0x3FF) | (((0) & 3) << 10);
            shadowOAM[21].attr2 = (((1) & 0xF) << 12) | (((2) * 32 + (5)) & 0x3FF) | (((0) & 3) << 10);
        } else {
            shadowOAM[20].attr2 = (((0) & 0xF) << 12) | (((swordfishBoss.frame * 4) * 32 + (7)) & 0x3FF) | (((0) & 3) << 10);
            shadowOAM[21].attr2 = (((0) & 0xF) << 12) | (((2) * 32 + (5)) & 0x3FF) | (((0) & 3) << 10);
        }
    } else {
...
void updateTorpedo() {
    if ((!(~(oldButtons) & ((1<<0))) && (~(buttons) & ((1<<0)))) && !torpedo.active) {
        spawnTorpedo();
        playSoundB(shot_data, shot_length - 100, 0);
    }
    if (torpedo.active) {
        torpedo.x += torpedoFacing * 5;
        if (torpedo.x + torpedo.width < 0 || torpedo.x > 512 ||
            abs(torpedo.x - player.x) > 240 / 2) {
                torpedo.active = 0;
            }
    }
}
...
void drawTorpedo() {
    if (torpedo.active) {
        if (!cheatEnabled) {
            shadowOAM[1].attr0 = ((torpedo.y - vOff) & 0xFF) | (0<<13) | (0<<14);
            shadowOAM[1].attr1 = ((torpedo.x - hOff) & 0x1FF) | (0<<14);
            shadowOAM[1].attr2 = (((0) & 0xF) << 12) | (((8) * 32 + (0)) & 0x3FF);
        } else {
            shadowOAM[1].attr0 = ((torpedo.y - vOff) & 0xFF) | (0<<13) | (1<<14);
            if (torpedoFacing == -1) {
                shadowOAM[1].attr1 = ((torpedo.x - hOff) & 0x1FF) | (2<<14) | (1<<12);
            } else {
                shadowOAM[1].attr1 = ((torpedo.x - hOff) & 0x1FF) | (2<<14);
            }
            shadowOAM[1].attr2 = (((0) & 0xF) << 12) | (((9) * 32 + (0)) & 0x3FF);
        }
    } else {
...
void drawShark() {
    ...
    for (int i = 0; i < 3; i ++) {
        if (sharks[i].active == 1) {
            if (sharks[i].y + sharks[i].height > vOff && sharks[i].y < 160 + vOff) {
                shadowOAM[shadowOAMIndex + i].attr0 = ((sharks[i].y - vOff) & 0xFF) | (0<<13) | (1<<14);
                shadowOAM[shadowOAMIndex + i].attr1 = ((sharks[i].x - hOff) & 0x1FF) | (3<<14);
                if (sharks[i].direction == RIGHT) {
                    shadowOAM[shadowOAMIndex + i].attr1 = shadowOAM[shadowOAMIndex + i].attr1 = ((sharks[i].x - hOff) & 0x1FF) | (3<<14) | (1<<12);
                }
                if (sharks[i].hurt) {
                    shadowOAM[shadowOAMIndex + i].attr2 = (((1) & 0xF) << 12) | (((sharks[i].frame * 4) * 32 + (15)) & 0x3FF);
                } else {
                    shadowOAM[shadowOAMIndex + i].attr2 = (((0) & 0xF) << 12) | (((sharks[i].frame * 4) * 32 + (15)) & 0x3FF);
                }
            }
        } else {
            shadowOAM[shadowOAMIndex + i].attr0 = (2<<8);
        }
    }
}
